...
async def callback_handler(update, context):
    ...
    # No limit on likes per user - append emoji directly
...
async def add_meme(update, context):
    ...
    if not update.message or not update.message.reply_to_message:
        await context.bot.send_message(chat_id, "Reply to a photo or document with /add")
        return
...
async def main():
    ...
    logger.info(f"Server listening on port {os.getenv('PORT', '10000')}")
    ...
    if WEBHOOK_URL:
        ...
        logger.error("Webhook URL not set - please set WEBHOOK_BASE")
