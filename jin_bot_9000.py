...
def save_group_id(chat_id: int):
    try:
        with open(group_file, "w") as f:
            f.write(str(chat_id))
        logger.info(f"Group chat ID {chat_id} saved.")
    except Exception as e:
...
async def send_meme(chat_id: int, context: Optional[ContextTypes.DEFAULT], randomize=False):
...
async def handle_like(update: Update, context: ContextTypes.DEFAULT):
...
async def set_interval(update: Update, context: ContextTypes.DEFAULT):
...
async def add_meme(update: Update, context: ContextTypes.DEFAULT):
    chat_id = update.effective_chat.id
    msg = update.message
...
async def get_group_id(update: Update, context: ContextTypes.DEFAULT):
    chat = update.effective_chat
...
async def init_group(update: Update, context: ContextTypes.DEFAULT):
    chat = update.effective_chat
...
async def main():
    ...
    app.add_handler(CommandHandler("start", send_meme_http))
