...
WEBHOOK_BASE = os.getenv("WEBHOOK_BASE", "").strip()  # example: https://your-app.onrender.com
...
RECENT_MEMES_MAX = 5
post_interval = 10  # Interval for scheduled posts in minutes
...
def load_likes():
    ...
    if likes_file.exists():
        try:
            ...
            logger.warning(f"Failed to load likes data: {e}")
...
async def set_interval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    job = context.job_queue.run_repeating(scheduled_post, interval=post_interval * 60, first=5, data=group_chat_id)
